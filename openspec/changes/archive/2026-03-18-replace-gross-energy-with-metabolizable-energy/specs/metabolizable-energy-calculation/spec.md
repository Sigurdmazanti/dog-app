...
### Requirement: Gross energy calculation
**Reason**: Replaced by `calculateMetabolizableEnergy`. Gross energy does not account for digestive losses and produces values unsuitable for pet-food nutritional comparison.
**Migration**: Replace all imports and call-sites of `calculateGrossEnergy` with `calculateMetabolizableEnergy`. The function signature is identical.
