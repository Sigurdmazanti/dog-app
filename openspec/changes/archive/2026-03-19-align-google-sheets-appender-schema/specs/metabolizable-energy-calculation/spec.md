...
where NFE is derived from `calculateNFE(data)`. The function SHALL accept a `NutritionData` object and return a rounded integer. When present in export rows, the computed value MUST be written to `item_kj_per_100g` in the canonical Google Sheets schema position and MUST NOT shift adjacent nutrient columns.
...
#### Scenario: Exported ME placement is stable
- **WHEN** a product row includes a computed metabolizable energy value
- **THEN** that value MUST be appended in `item_kj_per_100g` and all subsequent nutrient values SHALL remain aligned to their canonical columns
