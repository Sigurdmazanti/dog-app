...
The scraper SHALL extract the product title, ingredients description, and composition text from product pages on `calibrastore.co.uk` using Cheerio selectors targeting `h1.product-title` and the "Composition" accordion panel.

#### Scenario: Title is extracted
- **WHEN** a `calibrastore.co.uk` product page is scraped
...
A file at `scraper/sources/calibra.selectors.md` SHALL document the homepage URL (`https://calibrastore.co.uk`), a representative HTML snippet, and the selectors used to extract the title, ingredients description, and composition text from `calibrastore.co.uk` product pages.
