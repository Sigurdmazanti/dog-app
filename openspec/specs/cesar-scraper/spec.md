# cesar-scraper Specification

## Purpose

Scrape product data (title, ingredients description, composition text) from Cesar (cesar.com) dog food product pages. Cesar product pages currently present nutrition information as embedded images rather than HTML text, so ingredient and composition extractors are intentionally no-ops pending future image-based extraction.
