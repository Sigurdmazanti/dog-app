...
Define the scraping behaviour, source configuration, and selector reference for Essential Foods products on `essentialfoods.com`.
...
#### Scenario: Excluded bundle and accessory URLs are in discovery.ignore
- **WHEN** `essentialfoods.json` is read
- **THEN** the `discovery.ignore` array SHALL contain all bundle, taste-box, and accessory URLs (e.g. licking mat, pate taste boxes, mixed meals boxes, oil bundles, explorer boxes, finest box, training box, delight box) so the discovery crawler skips them
