...
Define the `misc` food type as a valid FoodType enum value, its default water percentage, and its integration with source JSON loading and Google Sheets output.
...
#### Scenario: misc products are loaded from source JSON
- **WHEN** a JSON source file contains a `misc:` key under `products`
