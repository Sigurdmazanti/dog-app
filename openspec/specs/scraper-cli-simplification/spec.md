...
The system MUST default the food type to `dry` when not explicitly provided, except when using a JSON source file via `--urls` where the food type SHALL be inferred from the JSON structure if `--food-type` is omitted.
...
#### Scenario: No food type argument with JSON source file
- **WHEN** the CLI is invoked with `--urls sources/brand.json` and no `--food-type` flag
- **THEN** the system loads all food-type categories from the JSON and pairs each URL with its category's food type

#### Scenario: Explicit food type override
- **WHEN** the CLI is invoked with `--food-type wet`
- **THEN** the system uses `wet` as the food type

#### Scenario: Explicit food type with JSON source file
- **WHEN** the CLI is invoked with `--urls sources/brand.json --food-type dry`
...
#### Scenario: No food type argument with non-JSON URL list
